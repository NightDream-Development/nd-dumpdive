...
function ProgressBar(ent)
    QBCore.Functions.Progressbar('diving_in_dumpster', Lang:t('progress.diving'), Config.ProgressTime * 1000, false, true, {
        ...
        TriggerServerEvent('nd-dumping:server:SetEntity', ObjToNet(ent), true)
        if not Config.ResetOnReboot then TriggerServerEvent('nd-dumping:server:ResetEntity', ObjToNet(ent)) end
        isBusy = false
