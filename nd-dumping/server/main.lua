...
RegisterNetEvent('nd-dumping:server:ResetEntity', function(entity)
    entities[entity] = 0
end)
...
RegisterNetEvent('nd-dumping:server:SetEntity', function(netId, isFinished)
    entities[netId] = 0
...
QBCore.Functions.CreateCallback('nd-dumping:server:getEntityState', function(source, cb, netId)
...
AddEventHandler('onResourceStop', function(resName)
    ...
    for _,v in pairs(entities) do
        if v == -1 then
            TriggerClientEvent('nd-dumping:client:ResetEntity', -1, _)
        end
    end
end)